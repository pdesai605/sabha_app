...
import { greetings } from "@/modules/office-desk/data/office-data";
...
import type { GreetingCategory, GreetingStatus } from "@/modules/office-desk/types";
...
export function GreetingsManagement() {
  ...
  const [selected, setSelected] = React.useState<typeof greetings[0] | null>(null);
  ...
  const byCategory = (list: typeof greetings) =>
    category === "all" ? list : list.filter((g) => g.category === category);

  const openComposer = (greeting: typeof greetings[0]) => {
